...
static	void _load_texture_data(t_texture *tex)
{
	...
	while (y < TEXTURE_SIZE)
	{
		...
		{
			tex->pixels[y][x] = px_decode_pixel(&tex->mlx_texture->pixels
				[i * tex->mlx_texture->bytes_per_pixel]);
			x++;
			i++;
		}
		x = 0;
	...
	mlx_delete_texture(tex->mlx_texture);
}
...
void	wall_setup(t_datamodel *dm)
{
	...
	while (i < TEXTURE_COUNT)
	{
		...
		if (i == WALL_N)
			dm->wall_tex[i]->mlx_texture = mlx_load_png("res/north_texture.png");
		else if(i == WALL_E)
			dm->wall_tex[i]->mlx_texture = mlx_load_png("res/east_texture.png");
		else if(i == WALL_S)
			dm->wall_tex[i]->mlx_texture = mlx_load_png("res/south_texture.png");
		else if (i == WALL_W)
			dm->wall_tex[i]->mlx_texture = mlx_load_png("res/west_texture.png");
