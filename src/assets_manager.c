...
static void	_load_textures( t_datamodel *datamodel, char *buffer, int fd)
{
	int		i;
	...
	while (read(fd, &buffer[i], 1))
	{
		...
		if (buffer[i] == '\n')
		{
			if (buffer[0] == 'N' && buffer[1] == 'O')
				datamodel->no_tex_path = _load_textures_path(&buffer[OFFSET]);
			if (buffer[0] == 'S' && buffer[1] == 'O')
				datamodel->so_tex_path = _load_textures_path(&buffer[OFFSET]);
			if (buffer[0] == 'W' && buffer[1] == 'E')
				datamodel->we_tex_path = _load_textures_path(&buffer[OFFSET]);
			if (buffer[0] == 'E' && buffer[1] == 'A')
				datamodel->ea_tex_path = _load_textures_path(&buffer[OFFSET]);
...
static void	_load_map_data(t_datamodel *datamodel, t_list *row_list)
{
	...
	i = 0;
	datamodel->tilemap->map = __calloc(datamodel->tilemap->size.y,
			sizeof(char *));
...
t_datamodel	*load_cub(char *filename)
{
	...
	_load_textures(datamodel, buffer, fd);
